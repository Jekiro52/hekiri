...
// Fetch all subjects
$subjects = $pdo->query(
    "SELECT s.*, st.strand_name, yl.year_level, sem.semester_name FROM subjects s
    JOIN strands st ON s.strand_id=st.id
    JOIN year_levels yl ON s.year_level_id=yl.id
    JOIN semesters sem ON s.semester_id=sem.id
    WHERE s.archived=0
    ORDER BY st.strand_name, yl.year_level, sem.semester_name, s.subject_name"
)->fetchAll();
...
<form method="post" class="mb-3 row g-2 align-items-end">
    ...
    <div class="col-md-1">
        ...
        <a href="registrar-assign_subjects.php" class="btn btn-secondary">Cancel</a>
    </div>
</form>
<?php endif; ?>
